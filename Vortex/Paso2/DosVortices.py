...
def main():
    ...
    plot_cada = 10       # Cada cuanto se toma una imagen de la simulación

    Nx = 1000             # Número de sitios en dirección x
    Ny = 300             # Número de sitios en dirección y
    Nt = 5000           # Número de pasos de tiempo
    ...
    figure, (ax1,ax2) = pyplot.subplots(1, 2)      # Para plotear más adelante

    #Frame = np.ones((Ny, Nx, Nt//plot_cada))

    def cicloPrincipal(F, obstaculo, It):       # Se define como función porque se hará la simulación por partes
                                                       # y se debe usar el ciclo principal en cada parte.
        ...
        for it in range(It):
            ...
            if(it%plot_cada == 0):
                ...
                magnitud = np.sqrt(ux**2+uy**2)                       # Magnitud de la velocidad


                #im1 = ax1.imshow(rotacional, cmap="bwr")
                #im2 = ax2.imshow(magnitud)
    ...
    ### Primer Parte de la implementación
    ## Se hace en el sistema de referencia del plato.
    ## Se hace incidir un flujo con velocidad horizontal constante contra el obstaculo plano.

    ### Condiciones iniciales

    F = np.ones((Ny, Nx, Nu)) #+ 0.01*np.random.randn(Ny, Nx, Nu)       # La cuadrícula. Cada sitio tiene Nu velocidades
                                                                                   # y se añade un ruido aleatorio en la inicialización.

    F[:, :, 7] = 2.0           # Velocidad inicial del fluido en el sri del plato, tiene dirección 7 (hacia la izquierda).
    ...
    for y in range(0, Ny):
        for x in range(0, Nx):
            if(distancia(x,Ny//2, x, y) < 40 and distancia(Nx//10,y, x, y) < 2):
                obstaculo[y][x] = True

    for y in range(0, Ny):
        for x in range(0, Nx):
            if(distancia(x,Ny//2, x, y) < 40 and distancia(50+Nx//10,y, x, y) < 2):
                obstaculo[y][x] = True

    F, Frame = cicloPrincipal(F, obstaculo, 300)

    ### Segunda Parte de la implementación
    ## Se vuelve al sistema de referencia del laboratorio.
    ## Se hace sustrae la velocidad horizontal constante del flujo.

    F[:, :, 7] = F[:, :, 7] - 2.0

    obstaculo = np.full((Ny,Nx), False)          # Se elimina el plano
                                                 # Se pone una pared al final del recorrido de los vórtices
    for y in range(0, Ny):
        for x in range(0, Nx):
            if(distancia(x,Ny//2, x, y) < Ny and distancia(Nx-5,y, x, y) < 4.0):
                obstaculo[y][x] = False

    F, Frame1 = cicloPrincipal(F, obstaculo, Nt)

    #print(Frame[0], len(Frame))

    fig = pyplot.figure()

    myimages = []

    for frm in range(len(Frame)):
        flux = pyplot.imshow(Frame[frm])#, cmap="bwr")
        myimages.append([flux])


    for frm in range(len(Frame1)):
        flux = pyplot.imshow(Frame1[frm])#, cmap="bwr")
        myimages.append([flux])


    my_anim = animation.ArtistAnimation(fig, myimages, interval=10, blit=True, repeat_delay=1000)
    ...
    my_anim.save("2vortex2D_long.gif", writer = writergif)
