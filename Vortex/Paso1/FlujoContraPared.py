...
def main():
    ...
    Nx = 250             # Número de sitios en dirección x
    Ny = 250             # Número de sitios en dirección y
    ...
    def cicloPrincipal(F, obstaculo, It):       # Se define como función porque se hará la simulación por partes
        ...
        for it in range(It):
            ...
            if(it%plot_cada == 0):
                ...
                Frm.append(rotacional)
                #np.append(Frm, magnitud)
    ...
    for y in range(0, Ny):
        for x in range(0, Nx):
            if(distancia(x,Ny//2, x, y) < 30 and distancia(Nx//4,y, x, y) < 2):
                obstaculo[y][x] = True
    ...
    for y in range(0, Ny):
        for x in range(0, Nx):
            if(distancia(x,Ny//2, x, y) < 200 and distancia(Nx-5,y, x, y) < 4.0):
                obstaculo[y][x] = True
    ...
    my_anim.save("vortex2D.gif", writer = writergif)
